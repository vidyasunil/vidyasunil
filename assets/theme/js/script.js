...
(function(a) {
	a.extend(a.easing, {
		easeInOutCubic : function(a, c, d, e, f) {
	...
	(function(a, c) {
		var d = function(a, b, c) {
			...
			return function() {
				...
				d ? clearTimeout(d) : c && a.apply(h, k);
	...
	var v = function() {
		var b = a('<div style="height: 50vh; position: absolute; top: -1000px; left: -1000px;">').appendTo("body"), c = b[0], d = parseInt(window.innerHeight / 2, 10), c = parseInt((window.getComputedStyle ? getComputedStyle(c, null) : c.currentStyle).height, 10);
		b.remove();
		return c == d
	}();
	a(function() {
		...
		}(a(window).width(), a(window).height()) : v || (a(window).smartresize(function() {
		...
		a.fn.jarallax && !a.isMobile() && (a(document).on("destroy.parallax", function(b) {
			...
			a(b.target).outerFind(".mbr-parallax-background").jarallax({
				speed : .6
			}).css("position", "relative")
		}));
		if (a.fn.socialLikes)
			a(document).on("add.cards", function(b) {
				a(b.target).outerFind(".mbr-social-likes:not(.mbr-added)").on("counter.social-likes", function(b, c, d) {
					999 < d && a(".social-likes__counter", b.target).html(Math.floor(d / 1E3) + "k")
				}).socialLikes({
					initHtml : !1
				})
			});
		var c, d, e = 0, f = null, l = !a.isMobile();
		a(window).scroll(function() {
			d && clearTimeout(d);
			var b = a(window).scrollTop(), c = b <= e || l;
			e = b;
			if (f) {
				var k = b > f.breakPoint;
				c ? k != f.fixed && ( l ? (f.fixed = k, a(f.elm).toggleClass("is-fixed")) : d = setTimeout(function() {
					f.fixed = k;
		...
		a(document).on("add.cards", function(b) {
			a(b.target).outerFind("[data-bg-video]").each(function() {
				for (var b, c = a(this).data("bg-video"), d = [/\?v=([^&]+)/, /(?:embed|\.be)\/([-a-z0-9_]+)/i, /^([-a-z0-9_]+)$/i], e = 0; e < d.length; e++)
					if ( b = d[e].exec(c)) {
						var c = "http" + ("https:" == location.protocol ? "s" : "") + ":", c = c + ("//img.youtube.com/vi/" + b[1] + "/maxresdefault.jpg"), f = a('<div class="mbr-background-video-preview">').hide().css({
							backgroundSize : "cover",
							backgroundPosition : "center"
						});
						a("> *:eq(0)", this).before(f);
						a("<img>").on("load", function() {
							if (120 == (this.naturalWidth || this.width)) {
								...
								f.css("background-image", 'url("' + this.src + '")').show()
						}).attr("src", c);
						a.fn.YTPlayer && !a.isMobile() && ( c = eval("(" + (a(this).data("bg-video-params") || "{}") + ")"), a("> *:eq(1)", this).before('<div class="mbr-background-video"></div>').prev().YTPlayer(a.extend({
							videoURL : b[1],
							containment : "self",
							showControls : !1,
							mute : !0
		...
		a("html").hasClass("is-builder") || a(document).click(function(b) {
			try {
				var c = b.target;
				if (!a(c).parents().hasClass("testimonials1")) {
					do
						if (c.hash) {
							var d = /#bottom|#top/g.test(c.hash);
							a( d ? "body" : c.hash).each(function() {
								b.preventDefault();
								var d = a(".mbr-navbar--sticky").length ? 64 : 0, d = "#bottom" == c.hash ? a(this).height() - a(window).height() : a(this).offset().top - d;
		...
		a(".cols-same-height .mbr-figure").each(function() {
			...
			var c = a(this), d = c.children("img"), e = c.parent(), f = d[0].width, l = d[0].height;
			d.one("load", function() {
				f = d[0].width;
				l = d[0].height;
				b()
	...
	a("html").hasClass("is-builder") || (a(document).ready(function() {
		if (!a.isMobile() && a("input[name=animation]").length) {
			...
			var c = a("p, h1, h2, h3, h4, h5, a, button, small, img, li, blockquote, .mbr-author-name, em, label, input, textarea, .input-group, .iconbox, .btn-social, .mbr-figure, .mbr-gallery, .mbr-slider, .mbr-map, .mbr-testimonial .card-block, .mbr-price-value, .mbr-price-figure, .dataTable, .dataTables_info").not(function() {
				return a(this).parents().is(".navbar, .mbr-arrow, footer, .iconbox, .mbr-slider, .mbr-gallery, .mbr-testimonial .card-block, #cookiesdirective, .mbr-wowslider, .accordion, .tab-content, .engine")
			}).addClass("hidden animated"), d = a(window);
			d.on("scroll resize", function() {
				var d = document.documentElement.scrollTop || document.body.scrollTop, f = d + window.innerHeight - 50;
				a.each(c, function() {
					var c = a(this), g = c[0], h = g.offsetHeight, g = b(g);
					if (g + h >= d && g <= f && c.hasClass("hidden"))
						c.removeClass("hidden").addClass("fadeInUp").one("webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend", function() {
	...
	a(document).ready(function() {
		a(".jsNewsCard").each(function() {
			a(this).on("click", function() {
				var b = a(this).attr("modal-id");
				a(b).modal("show")
			})
		})
	});
	...
	initCountdown = function() {
		a(".countdown:not(.countdown-inited)").each(function() {
			a(this).addClass("countdown-inited").countdown(a(this).attr("data-end"), function(b) {
				a(this).html(b.strftime('<div class="row"><div class="col-xs-12 col-sm-6 col-lg-3"><span class="number-wrap"><span class="number">%D</span><span class="period">Days</span><div class="bottom1"></div><div class="bottom2"></div><span class="dot">:</span></span></div><div class="col-xs-12 col-sm-6 col-lg-3"><span class="number-wrap"><span class="number">%H</span><span class="period">Hours</span><div class="bottom1"></div><div class="bottom2"></div><span class="dot">:</span></span></div><div class="col-xs-12 col-sm-6 col-lg-3"><span class="number-wrap"><span class="number">%M</span><span class="period">Minutes</span><div class="bottom1"></div><div class="bottom2"></div><span class="dot">:</span></span></div><div class="col-xs-12 col-sm-6 col-lg-3"><span class="number-wrap"><span class="number">%S</span><span class="period">Seconds</span><div class="bottom1"></div><div class="bottom2"></div></span></div></div>'))
			})
		});
		a(".countdown:not(.countdown-inited)").each(function() {
				...
				if (a(b.target).hasClass("extTestimonials3")) {
					...
					a(b.target).find(".carousel-item").each(function() {
						for (var b = a(this), e = 1; e < c; e++) {
							...
							var f = b.index();
							b.find(".col-md-12:first").clone(!0).addClass("cloneditem-" + e).addClass("clonedCol").attr("data-cloned-index", f).appendTo(a(this).children().eq(0))
						}
					})
				}
	...
	a("html").hasClass("is-builder") || (0 < a("section.extTestimonials1:not(.extTestimonials5)").length && (a(window).on("resize", function() {
		a("section.extTestimonials1:not(.extTestimonials5)").each(function() {
			...
			a(this).find(".carousel-item").each(function() {
				a(this).find(".card-block").css("min-height", d + "px")
			})
		})
	}), setTimeout(function() {
		a(window).trigger("resize")
	}, 100)), a(document).on("add.cards change.cards", function(b) {
		if (a(b.target).hasClass("extTestimonials1") || a(b.target).hasClass("extTestimonials3"))
			if ((0 < window.navigator.userAgent.indexOf("MSIE ") || navigator.userAgent.match(/Trident.*rv\:11\./)) && a(b.target).find(".card-block").each(function() {
				...
				2 > c || (a(b.target).find(".carousel-inner").attr("class", "carousel-inner slides" + c), a(b.target).find(".carousel-item .col-md-12").each(function() {
					...
					for (var b = a(this), e = 1; e < c; e++)
						b = b.next(), b.length || ( b = a(this).siblings(":first")), b.find(".col-md-12:first").clone().addClass("cloneditem-" + e).appendTo(a(this).children().eq(0))
				}))
			}
	}), a(document).on("add.cards change.cards", function(b) {
		b = a(b.target);
		if (b.hasClass("table1")) {
			var c = b.find(".table-wrapper"), d = "true" === c.attr("data-search");
			if (d)
				var e = c.attr("search-text"), f = c.attr("info1-text"), l = c.attr("info2-text"), g = c.attr("info3-text"), h = c.attr("info4-text");
			b.find(".table").DataTable({
				...
				language : {
					...
					info : f + " _END_ " + l,
					infoEmpty : f + " _END_ " + l,
					infoFiltered : g + " _MAX_ " + h
				}
			})
		}
	}));
	filterShop = function(b, c) {
		var d = 3 > c ? a(".shop-items").children().sort(function(b, d) {
		...
		for (var e = 0; e < d.length; e++)
			a(".shop-items").append(d[e]);
		modalEvents()
	};
	if (0 != a(".shop1").length) {
		var w = a(".shop-items").children(), m = a(".shop-items").children(), p = a(".sort-buttons .filter-by-pu .btn"), q = a(".sort-buttons .filter-by-pd .btn"), t = a(".sort-buttons .filter-by-d .btn");
		...
		a(".filter-by-pd").on("click", function() {
			a(q).removeClass("disableSortButton");
		...
		a(".price-range").on("click", function() {
			...
			a(".mbr-gallery-item:not(.bestsellers .mbr-gallery-item)").each(function(d, e) {
				parseFloat(a(e).attr("price")) >= parseFloat(b) && parseFloat(a(e).attr("price")) <= parseFloat(c) ? a(e).removeClass("hided-by-price") : a(e).addClass("hided-by-price")
			})
		});
	...
	var x = a(".mbr-gallery-filter-all");
	a(document).on("add.cards change.cards", function(b) {
		var c = a(b.target);
		if (c.hasClass("shop1")) {
			var d = [];
			...
			if (0 < c.find(".mbr-gallery-filter").length && a(b.target).find(".mbr-gallery-filter").hasClass("gallery-filter-active")) {
				...
				c.find(".mbr-gallery-filter ul").append(x).append(e);
				...
				c.find(".mbr-gallery-item__hided").removeClass("mbr-gallery-item__hided"), c.find(".mbr-gallery-row").trigger("filter")
		}
	});
	findMaxItemPrice = function() {
		var b = 0;
		a(".mbr-gallery-item").each(function(c, d) {
			parseFloat(a(d).attr("price")) > b && ( b = parseFloat(a(d).attr("price")))
		});
		return b
	};
	...
	rangeSliderInit = function() {
		var b = a("input.min-input"), c = a("input.max-input"), d = a("div.range-controls"), e = d.find("div.bar"), f = d.find("div.min-toggle"), l = d.find("div.max-toggle"), g = e.innerWidth() - 20, h = 0, k = g, n = d.offset().left, m = b.val(), r = c.val(), p = b.val(), q = parseInt(a(".filter-cost").width()) - 20;
		f.mousedown(function() {
			a(document).on("mousemove", function(c) {
				l.css("z-index", 0);
				f.css("z-index", 1);
				togglePos = Math.round(c.pageX - n) - 10;
				h = 0 >= togglePos ? 0 : togglePos >= g ? g : togglePos;
				f.css({
					left : function(a, b) {
						if (h < k)
							return m = h;
				...
				e.css({
					...
					width : function() {
						return h < k ? g - (h + (g - k)) : 0
					}
				});
				a(b).val(Math.floor((findMaxItemPrice() - findMinItemPrice()) / q * m) + parseInt(p));
				autoPriceRange()
			})
		});
		l.mousedown(function() {
			a(document).on("mousemove", function(b) {
				l.css("z-index", 1);
				f.css("z-index", 0);
				togglePos = Math.round(b.pageX - n) - 10;
				k = 0 >= togglePos ? 0 : togglePos >= g ? g : togglePos;
				l.css({
					right : function(a, b) {
						if (h < k)
							return r = k, g - k;
				...
				a(c).val(Math.ceil((findMaxItemPrice() - findMinItemPrice()) / q * r) + parseInt(p));
				autoPriceRange()
			})
		});
	...
	0 != a(".shop1").length && a(document).ready(function() {
	...
	if (!a("html").hasClass("is-builder")) {
		moveToModal = function(b) {
			var c = a(".shopItemsModal"), d = a(b).find(".sidebar_wraper").clone(), e = a(b).find("img").clone();
			b = a(b).find(".onsale").clone();
			a(c).children(".text-modal").append(d);
			a(c).children(".image-modal").append(e).append(b);
			d = a(c).children(".text-modal").find(".price-block");
			a(c).children(".text-modal").find(".price-block").remove();
			a(c).children(".text-modal").find(".card-description").after(d)
		};
		cleanModal = function() {
		...
		modalEvents = function() {
			a(".shop1 .mbr-gallery-item .galleryItem .item_overlay").on("click", function(b) {
		...
		0 != a(".shop1").length && (a(document).ready(function() {
			m = a(".shop-items").children();
			filterShop(m, 3)
		}), null !== navigator.userAgent.match(/(iPad)|(iPhone)|(iPod)|(Android)|(PlayBook)|(BB10)|(BlackBerry)|(Opera Mini)|(IEMobile)|(webOS)|(MeeGo)/i) ||
		void 0 !== document.createTouch || "ontouchstart" in window || "onmsgesturechange" in window || navigator.msMaxTouchPoints ? (a(".range-controls").css("display", "none"), a(".price-controls, .filter-cost").css("margin-bottom", "15px")) : (a("input.min-input, input.max-input").prop("disabled", !0), a(".filterPriceRange").css("display", "none")));
		var n = function() {
			var b = a(".shop1");
			b.length && a("body").stop().animate({
				scrollTop : b.offset().top
			}, 800, "linear")
		};
		if (1100 > window.screen.width && 0 != a(".shop1").length)
			a(".showAll, .filterPriceRange").on("click", n)
	}
	move = function(b) {
		b.width();
	...
	if (!document.getElementById("top-1")) {
		...
		a.id = "top-1";
		a.className = "engine";
		a.innerHTML = '<a href="https://mobirise.com">mobirise.com</a> Mobirise v3.10.8';
